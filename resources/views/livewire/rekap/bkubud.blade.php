<div class="p-4">
    ...
    <div class="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-7 gap-2">
        ...
        @php
        $dataList = $bkubudData[$hari['tanggal_penuh']] ?? collect([]);
        ...
        <div wire:click="showDetail('{{ $hari['tanggal_penuh'] }}')"
            class="border p-2 text-center cursor-pointer hover:bg-blue-100 
                {{ $detailTanggal == $hari['tanggal_penuh'] ? 'bg-blue-200' : '' }} transition dark:border-gray-600 dark:hover:bg-gray-700">
            <div class="font-bold dark:text-white">{{ $hari['tanggal'] }}</div>
    ...
    {{-- Detail Data --}}
    @if ($detailTanggal)
    <div class="mt-4 p-4 border bg-gray-100 overflow-x-auto rounded shadow dark:bg-gray-700 dark:border-gray-600">
        <h3 class="font-semibold mb-2 dark:text-white">Detail BKUBUD untuk tanggal: {{ $detailTanggal }}</h3>
        <table class="w-full text-sm border-collapse">
            <thead class="bg-gray-200 dark:bg-gray-600">
                <tr>
                    <th class="p-2 border dark:border-gray-600 dark:text-white">Tanggal</th>
                    <th class="p-2 border dark:border-gray-600 dark:text-white">Nomor Bukti</th>
            ...
            <tbody>
                ...
                <tr class="bg-white hover:bg-gray-50 dark:bg-gray-800 dark:hover:bg-gray-700">
                    <td class="p-2 border dark:border-gray-600 dark:text-white">{{ $data->tanggal }}</td>
                    <td class="p-2 border dark:border-gray-600 dark:text-white">{{ $data->no_bukti }}</td>
                    <td class="p-2 border dark:border-gray-600 dark:text-white">{{ $data->uraian }}</td>
                    <td class="p-2 border text-green-600 dark:border-gray-600 dark:text-green-400">{{
                        Number::currency($data->penerimaan,
                        'IDR', 'id') }}</td>
                    <td class="p-2 border text-red-600 dark:border-gray-600 dark:text-red-400">{{
                        Number::currency($data->pengeluaran,
                        'IDR', 'id') }}</td>
                </tr>
                @empty
                <tr>
                    <td colspan="6" class="text-center text-gray-500 p-4 dark:text-gray-400">Tidak ada data untuk
                        tanggal ini.</td>
                </tr>
                @endforelse
            </tbody>
